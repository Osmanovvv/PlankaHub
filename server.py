...
class PlankaHandler(SimpleHTTPRequestHandler):
    ...
    def handle_create_behavior_event(self) -> None:
        ...
        if event_type not in {"click", "scroll"}:
            event_type = "click"
    ...
    def handle_behavior_analytics(self) -> None:
        with sqlite3.connect(DB_PATH) as connection:
            ...
            recent_events = connection.execute(
                """
                SELECT id, session_id, event_type, event_name, label, target_text, target_href,
                       page_path, viewport_width, viewport_height, scroll_y, scroll_percent, created_at
                FROM behavior_events
